...
class HomeState extends State<HomeScreen> {
  ...
  if (existingSyllabus.docs.isNotEmpty) {
    ...
    if (!overwrite) {
      ...
      // Delete existing file and metadata
      try {
        final docId = existingSyllabus.docs.first.id;
        final existingData = existingSyllabus.docs.first.data();

        // Delete file from Firebase Storage
        if (existingData['fileURL'] != null) {
          final storageRef = FirebaseStorage.instance.refFromURL(existingData['fileURL']);
          await storageRef.delete();
        }

        // Delete metadata from Firestore
        await FirebaseFirestore.instance.collection('syllabi').doc(docId).delete();
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to delete existing syllabus: $e')),
        );
        return;
      }
    }
